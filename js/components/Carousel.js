export default function Hero() {
    ...
    // Aquí agregamos la parte decorativa de Navidad (por ejemplo, copos de nieve)
    const christmasDecoration = `
        <div class="absolute top-0 left-0 right-0 bottom-0 z-20 pointer-events-none">
            <div class="absolute top-0 left-0 w-full h-full bg-cover bg-no-repeat" style="background-image: url('images/navidad.png'); opacity: 0.3;"></div>
            <div class="absolute top-0 left-0 w-full h-full flex justify-between items-start">
                <!-- Puedes agregar imágenes de adornos, copos de nieve, luces aquí -->
    ...
    hero.innerHTML = `
        <div class="relative w-full h-[60vh] overflow-hidden" id="hero-container">
            ${christmasDecoration} <!-- Decoración navideña -->
